...
module.exports = class BasicGenerator extends Generator {
  ...
  writing() {
    ...
    this.fs.copy(this.templatePath('app', 'tsconfig.json'), this.destinationPath('tsconfig.json'));
    this.fs.copy(this.templatePath('app', 'typings.d.ts'), this.destinationPath('typings.d.ts'));
    this.fs.copy(this.templatePath('app', 'tslint.json'), this.destinationPath('tslint.json'));

    if (this.props.react.includes('dva')) {
      this.fs.copy(this.templatePath('app', 'src', 'models', '.*'), this.destinationPath('src/models'));
      this.fs.copyTpl(this.templatePath('app', 'src', 'app.tsx'), this.destinationPath('src/app.tsx'), context);
    }
  }
};
